/**
 * battery.ts — Key 1
 * Shows B1 (18650 UPS) and B2 (12V LiON) capacity.
...
export class BatteryStatus extends SingletonAction {
  ...
  override async onWillAppear(ev: WillAppearEvent): Promise<void> {
    ...
    // Remove before adding — prevents duplicate listeners on profile switches
  ...
  private async showNoData(): Promise<void> {
    for (const a of this.active) await a.setImage(noDataButton("BATTERY"));
  }
  ...
  private async renderTo(a: any, d: UPSData): Promise<void> {
    ...
    await a.setImage(makeButton(bg, [
      ...
      { text: `B1 ${b1.capacity}%${b1.acPresent ? " ⚡" : ""}`, y: 29, size: 15 },
