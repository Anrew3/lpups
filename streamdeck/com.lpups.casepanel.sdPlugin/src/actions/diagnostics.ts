...
export class Diagnostics extends SingletonAction {
  ...
  private async runDiagnostics(): Promise<void> {
    ...
    try {
      const { stdout } = await execAsync(`powershell.exe -NonInteractive -NoProfile -ExecutionPolicy Bypass -File "${SCRIPT}" -StateFile "${STATE_FILE}" -OutFile "${RESULT_FILE}"`);
  ...
  private async renderTo(a: any): Promise<void> {
    if (this.diagState === "IDLE") {
      return void a.setImage(await makeButton(C.BLUE, [
    ...
    if (this.diagState === "RUNNING") {
      return void a.setImage(await makeButton(C.GRAY, [
    ...
    await a.setImage(await makeButton(s.fail > 0 ? C.RED : s.warn > 0 ? C.YELLOW : C.GREEN, [
